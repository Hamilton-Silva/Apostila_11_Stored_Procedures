-- Criando tabelas e demais procedimentos necessários para a realização das tarefas
...
-- Ajustando o cadastro de clientes para registro no Log 
...
-- Ajustando a criação de pedido para registro no Log 
CREATE OR REPLACE PROCEDURE sp_criar_pedido (OUT cod_pedido INT, cod_cliente INT)
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Ajustando 'adicionar item ao pedido' para registro no Log
...
-- Ajustando 'calcular valor do pedido' para registro no Log
...
-- Ajustando 'fechar pedido' para registro no Log
...
-- Ajustando 'obter notas para compor troco' para registro no Log
...
-- Visualização do Log depois dos ajustes acima realizados
SELECT * FROM tb_log ORDER BY data_hora DESC;


-- Exercício 1.2
-- Criando o procedimento 'contar pedidos por cliente'
...
DECLARE
    total_pedidos INT;
BEGIN
    -- Conta o total de pedidos do cliente
    SELECT COUNT(*)
    INTO total_pedidos
    FROM tb_pedido
...
END;
$$;
-- Verificando o procedimento no Log
SELECT * FROM tb_log ORDER BY data_hora DESC;

-- Exercício 1.3
-- Criando o procdimento com parâmetro de saída
...
DECLARE
    total_pedidos INT;
BEGIN
    -- Contando o total de pedidos de um cliente
    SELECT COUNT(*)
    INTO total_pedidos
    FROM tb_pedido
    WHERE cod_cliente = cod_cliente;

    -- Atualizando o parâmetro INOUT em conformidade com o total de pedidos
...
-- Exercício 1.5
-- Criando um procedimento solicitado n atarefa 
...
BEGIN
    ...
    -- Montando uma mensagem com os nomes utilizados no cadastro
    mensagem := CONCAT(
        'Os clientes: ',
...
-- Exercício 1.6
-- Criando blocos anônimos para cada procdimento criando neste script
-- Cadastrando vários clientes e retornando uma mensagem com os respectivos nomes
DO $$
DECLARE
    resultado TEXT; 
BEGIN
...
END;
$$;
-- Retornando o total de pedidos realizados por um cliente
